...
module.exports = {
  ...
  defmodule ${projectName}.${modelCamelCase}Fixtures do
    ...
    def insert_all do
      ecto_array = Enum.map(${modelPlural}, fn(each_map) -> map_with_iso_string_parser(each_map) end)

      Repo.insert_all(${modelCamelCase}, ecto_array, returning: true)
    end

    def insert_by_id(id) do
      ecto_map = Enum.find(${modelPlural}, fn(each_map) ->
        each_map.id == id
      end) |> map_with_iso_string_parser

      struct(${modelCamelCase}, ecto_map) |> Repo.insert!
    end
  end
  `.trim());
